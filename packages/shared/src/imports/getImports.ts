...
const importMap =
  require('css-vuetify/dist/json/importMap.json') as typeof import('css-vuetify/dist/json/importMap.json');
const importMapLabs =
  require('css-vuetify/dist/json/importMap-labs.json') as typeof import('css-vuetify/dist/json/importMap-labs.json');
...
export function getImports(source: string, options: Options) {
  ...
  const ignore =
    (isObject(options.autoImport) && options.autoImport.ignore) || null;
  ...
  if (components.size || directives.size) {
    ...
    directives.forEach(directive => {
      if (
        importMap.directives.includes(directive.name) &&
        !ignore?.includes(directive.name as any)
      ) {
  ...
  resolvedComponents.forEach(component => {
    addImport(
      ...
      component.name,
      component.symbol,
      'vuetify/lib/' + (map.components as any)[component.name].from
    );
  });
